...
export function Header() {
  ...
  return (
    ...
    >
      ...
      <div className="bg-gradient-to-r from-[#1a8f4a] via-[#D4AF37] to-[#1a8f4a] py-2 px-2">
        <div className="container mx-auto">
          <div className="flex items-center justify-center gap-3 sm:gap-6 md:gap-10 lg:gap-12 flex-wrap text-xs sm:text-sm">
            {promoItems.map((item, index) => (
              <div
                key={index}
                className="flex items-center gap-1.5 sm:gap-2 whitespace-nowrap text-white font-medium drop-shadow-sm"
              >
                <item.icon className="h-3.5 w-3.5 sm:h-4 sm:w-4 shrink-0 text-white" />
          ...
          <div className="flex items-center justify-between gap-2 sm:gap-4">
            ...
            <div className="flex items-center gap-1 sm:gap-2">
              ...
              {user ? (
                <DropdownMenu >
                  ...
                  <DropdownMenuContent align="end" className="w-56 bg-white border border-gray-200 shadow-xl">
                    <div className="px-3 py-2 text-sm text-gray-600 bg-gray-50 border-b border-gray-200 rounded-t-md">
                      {user.email}
                    </div>
                    {userRole === "admin" && (
                      <>
                    ...
                    <DropdownMenuItem asChild className="hover:bg-green-50">
                      <Link href="/account/dashboard" className="text-gray-700">Dashboard</Link>
                    </DropdownMenuItem>
                    <DropdownMenuItem asChild className="hover:bg-green-50">
                      <Link href="/account/orders" className="text-gray-700">My Orders</Link>
                    </DropdownMenuItem>
                    <DropdownMenuItem asChild className="hover:bg-green-50">
                      <Link href="/account/profile" className="text-gray-700">Profile Settings</Link>
                    </DropdownMenuItem>
                    <DropdownMenuItem asChild className="hover:bg-green-50">
                      <Link href="/wishlist" className="text-gray-700">My Wishlist</Link>
                    </DropdownMenuItem>
                    <DropdownMenuSeparator className="bg-gray-200" />
                    <DropdownMenuItem
              ...
              <Button
                variant="ghost"
                size="icon"
                className="lg:hidden h-9 w-9 sm:h-10 sm:w-10 bg-gradient-to-br from-[#1a8f4a] to-[#157a3d] hover:from-[#157a3d] hover:to-[#1a8f4a] rounded-full shadow-md hover:shadow-lg transition-all duration-300"
      ...
      >
        ...
        <div
          className={cn(
            "absolute top-0 left-0 right-0 bg-white shadow-xl max-h-[calc(100vh-114px)] overflow-y-auto transition-transform duration-300 border-b-4 border-[#D4AF37]",
                  ...
                  {link.hasDropdown ? (
                    <div>
                      <button
                        ...
                        className={cn(
                          "w-full flex items-center justify-between px-4 py-3 font-bold rounded-lg transition-all duration-200",
                          isActiveLink(link.href)
                            ? "bg-[#1a8f4a] text-white shadow-md"
                            : "text-gray-900 hover:bg-[#1a8f4a]/10 hover:text-[#1a8f4a]"
                        )}
                      >
                        {link.name}
                        <ChevronDown
                      ...
                      >
                        <ul className="ml-4 space-y-1 pb-2 border-l-3 border-[#D4AF37] bg-gray-50/50 rounded-r-lg">
                          <li>
                            <Link
                              href={link.href}
                              className="block px-4 py-3 text-[#1a8f4a] font-bold hover:bg-[#1a8f4a] hover:text-white rounded-lg transition-all duration-200"
                              onClick={() => setMobileMenuOpen(false)}
                            >
                              View All Products
                            </Link>
                          ...
                          {link.subLinks?.map((subLink) => (
                            <li key={subLink.name}>
                              <Link
                                href={subLink.href}
                                className="block px-4 py-3 text-gray-800 font-semibold hover:text-white hover:bg-[#1a8f4a] rounded-lg transition-all duration-200"
                                onClick={() => setMobileMenuOpen(false)}
                              >
                                {subLink.name}
                              </Link>
                    ...
                    <Link
                      href={link.href}
                      className={cn(
                        "block px-4 py-3 font-bold rounded-lg transition-all duration-200",
                        isActiveLink(link.href)
                          ? "bg-[#1a8f4a] text-white shadow-md"
                          : "text-gray-900 hover:bg-[#1a8f4a]/10 hover:text-[#1a8f4a]"
                      )}
            ...
            {!user && (
              <div className="mt-4 pt-4 border-t border-gray-200">
                <Link
                  ...
                  onClick={() => setMobileMenuOpen(false)}
                >
                  <Button className="w-full bg-gradient-to-r from-[#1a8f4a] to-[#D4AF37] hover:from-[#D4AF37] hover:to-[#1a8f4a] text-white font-semibold py-3 shadow-md">
                    Login / Sign Up
                  </Button>
                </Link>
              </div>
